...
function PartidoInner() {
  ...
  // --- alineación ---
  const [squad, setSquad] = useState<any[]>([])             // convocados / plantel del partido (puede incluir invitados)
  ...
  // Trae fichas de jugadores por id y etiqueta con su equipo a los que no son de este equipo
  async function fetchPlayersById(ids: string[]): Promise<any[]> {
    if (!ids.length) return []
    const { data } = await supabase.from('players').select('*').in('id', ids)
    const otros = (data || []).filter((p: any) => p.team_id && p.team_id !== teamId)
    const tIds = Array.from(new Set(otros.map((p: any) => p.team_id)))
    ...
    return (data || []).map((p: any) =>
      p.team_id && p.team_id !== teamId ? { ...p, team_name: names[p.team_id] || 'Otro equipo' } : p
    )
  }
  ...
  function addInvitado(p: JugadorInvitado) {
    setShowPicker(false)
    if (squad.some(s => s.id === p.id)) { showToast('Ya está en la convocatoria'); return }
    setSquad(s => [...s, p as any])
    setLuState(st => ({ ...st, [p.id]: 'bench' }))
    showToast(p.name + ' · al banquillo')
  }

  function quitarDelSquad(id: string) {
    setSquad(s => s.filter(p => p.id !== id))
    setLuState(st => { const n = { ...st }; delete n[id]; return n })
  }

  /* ---------- Selección/creación de partido ---------- */
  async function loadConvocados(mid: string, plList: Player[]): Promise<any[]> {
    ...
    if (conv && (conv as any).convocatoria_jugadores?.length) {
      const ids: string[] = (conv as any).convocatoria_jugadores
        .filter((j: any) => j.estado === 'convocado').map((j: any) => j.player_id)
      if (ids.length) {
        const propios = plList.filter(p => ids.includes(p.id))
        // convocados que no son de la plantilla de este equipo (invitados de otros equipos)
        const faltan = ids.filter(id => !plList.some(p => p.id === id))
        const invitados = await fetchPlayersById(faltan)
        const total = [...propios, ...invitados]
  ...
  async function resumeMatch() {
    ...
    const ids = Object.keys(resumeAvail.players)
    const propios = allPlayers.filter(p => ids.includes(p.id))
    const faltan = ids.filter(id => !allPlayers.some(p => p.id === id))
    const invitados = await fetchPlayersById(faltan)   // invitados de otros equipos
    const sq = [...propios, ...invitados]
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: 20 }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--gold)' }}>Alineación</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              ...
              {fieldPlayers.map(p => <LuItem key={p.id} p={p} side="field" onClick={() => toggleLu(p.id)} onRemove={p.team_name ? () => quitarDelSquad(p.id) : null} />)}
            </div>
            <div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 8 }}>Banquillo</div>
              {benchPlayers.map(p => <LuItem key={p.id} p={p} side="bench" onClick={() => toggleLu(p.id)} onRemove={p.team_name ? () => quitarDelSquad(p.id) : null} />)}
          ...
          <div style={{ width: 300, borderLeft: '1px solid var(--border)', background: 'var(--surface)', overflowY: 'auto', padding: 12 }}>
            ...
            {benchPlayers.map(p => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 8px', borderRadius: 11, marginBottom: 6, background: 'var(--surface2)' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600 }}>{p.name}{isGK(p) ? ' 🧤' : ''}<EtiquetaEquipo p={p} />{live.players[p.id].seconds ? <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}> · {minLbl(live.players[p.id].seconds)}</span> : ''}</div>
              ...
              {squad.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '8px 10px', fontWeight: 600 }}>{p.name}{isGK(p) ? ' 🧤' : ''}<EtiquetaEquipo p={p} /></td>
...
function EtiquetaEquipo({ p }: any) {
  if (!p?.team_name) return null
  return (
    <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.3px', color: 'var(--gold)', background: 'var(--surface3)', borderRadius: 5, padding: '1px 5px', marginLeft: 6, whiteSpace: 'nowrap' }}>
      {p.team_name}
    </span>
  )
}
function LuItem({ p, side, onClick, onRemove }: any) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface2)', border: '1px solid ' + (p.team_name ? 'var(--gold)' : 'var(--border)'), borderRadius: 10, padding: '7px 9px', marginBottom: 6 }}>
      <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0, cursor: 'pointer' }}>
        <div style={{ width: 28, height: 28, borderRadius: 7, background: 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: 'var(--gold)', fontSize: 12, flexShrink: 0 }}>{p.dorsal ?? '·'}</div>
        <div style={{ fontSize: 13, fontWeight: 600, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}{isGK(p) ? ' 🧤' : ''}<EtiquetaEquipo p={p} /></div>
      </div>
      {onRemove && (
        <button onClick={onRemove} title="Quitar de la convocatoria"
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: gk ? 'linear-gradient(90deg,rgba(91,184,232,.09),var(--surface))' : 'var(--surface)', border: '1px solid ' + (gk ? 'rgba(91,184,232,.45)' : 'var(--border)'), borderRadius: 14, padding: '8px 10px', marginBottom: 9 }}>
      ...
      <div style={{ minWidth: 0, flex: 1, cursor: 'pointer' }} onClick={onDetail}>
        <div style={{ fontWeight: 700, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}{gk ? ' 🧤' : ''}<EtiquetaEquipo p={p} /></div>
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <div><h2 style={{ margin: 0, fontSize: 20 }}>{p.name}{gk ? ' 🧤' : ''}</h2><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{p.team_name ? p.team_name + ' · ' : ''}{p.position || '—'} · {minLbl(lp.seconds)} jugados</div></div>
    ...
    <>
      ...
      {list.map((p: any) => (
        <div key={p.id} onClick={() => onPick(p.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px', borderRadius: 11, marginBottom: 6, background: 'var(--surface2)', cursor: 'pointer' }}>
          <div style={{ width: 30, height: 30, borderRadius: 8, background: 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: 'var(--gold)', fontSize: 13 }}>{p.dorsal ?? '·'}</div>
          <div style={{ fontSize: 14, fontWeight: 600 }}>{p.name}{isGK(p) ? ' 🧤' : ''}<EtiquetaEquipo p={p} />{minutesOf ? <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}> · {minutesOf(p.id)}</span> : ''}</div>
